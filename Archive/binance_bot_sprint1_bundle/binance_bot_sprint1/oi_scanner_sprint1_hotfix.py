...
class BinanceScanner:
    ...
    def save_pending(self, p: PendingSetup):
            ...
            if self.review_runtime:
                self._review_register_pending_case(asdict(p))
    ...
    def build_pending_long_setup(self, symbol: str) -> Optional[PendingSetup]:
        ...
        scanner_cfg = self.cfg["scanner"]

        bars_5m_all = self.klines(symbol, scanner_cfg["interval_5m"], limit=80)
        bars_15m_all = self.klines(symbol, scanner_cfg["interval_15m"], limit=20)
        bars_1h_all = self.klines(symbol, scanner_cfg["interval_1h"], limit=12)
        ...
        oi_jump = self.calc_oi_jump_pct(symbol)
        if oi_jump is None or oi_jump < float(th["oi_jump_pct_5m"]):
            return None

        body_ratio = self.candle_body_ratio(signal_bar)
        vol_ratio = self.volume_ratio(bars_5m)
        wick = self.wick_ratio(signal_bar)
        funding_pct = self.funding(symbol)
        trend15 = self.trend_15m(bars_15m)
        trend1h = self.trend_1h(bars_1h)
        ...
        if vol_ratio < min_vol_ratio or wick > max_wick_ratio or body_ratio < min_body_ratio:
            return None
        if self.already_open_signal(symbol, "LONG") or self.already_pending_setup(symbol, "LONG"):
            return None
        ...
        score = min(
            100.0,
            50 + oi_jump * 6 + (vol_ratio - 1) * 20 + body_ratio * 10
            - max(funding_pct, 0) * 120 - wick * 22
        )
    ...
    def build_pending_short_exhaustion_setup(self, symbol: str) -> Optional[PendingSetup]:
        ...
        scanner_cfg = self.cfg["scanner"]
        bars_15m_all = self.klines(symbol, scanner_cfg["interval_15m"], limit=60)
        bars_1h_all = self.klines(symbol, scanner_cfg["interval_1h"], limit=24)
        ...
        if self.already_open_signal(symbol, "SHORT") or self.already_pending_setup(symbol, "SHORT"):
            return None

        exhaustion = self.detect_1h_exhaustion(bars_1h)
        if not exhaustion:
            return None

        breakdown = self.detect_15m_breakdown_after_exhaustion(bars_15m)
        if not breakdown:
            return None

        funding_pct = self.funding(symbol)
        oi_change_15m = self.calc_oi_change_pct(symbol, period="15m", limit=3)
        ...
        confidence = max(0.0, min(0.99, score / 100.0))
        reason = (
            f"1h exhaustion {score_exhaustion:.0f}/35 + "
            f"15m breakdown {score_breakout:.0f}/30 -> retest fail pending"
        )
    ...
    def scan_once(self):
        self.round_idx += 1
        print(f"[round start] idx={self.round_idx} time_ms={int(time.time() * 1000)}")
        ...
        if confirmed:
            ...
            chosen = confirmed[:top_n]
            for s in chosen:
                self.save_signal(s)
                pending_rows = self.read_csv(self.pending_file)
                changed = False
                for prow in pending_rows:
                    if prow.get("setup_id", prow.get("pending_id", "")) == s.setup_id:
                        prow["send_decision"] = "SENT"
                        prow["skip_reason"] = ""
                        changed = True
                        break
                if changed:
                    self.write_csv(self.pending_file, pending_rows, fieldnames=self.pending_fields)
            ...
            for s in confirmed[top_n:]:
                skipped_top_n += 1
                pending_rows = self.read_csv(self.pending_file)
                changed = False
                for prow in pending_rows:
                    if prow.get("setup_id", prow.get("pending_id", "")) == s.setup_id:
                        prow["send_decision"] = "SKIPPED_TOP_N"
                        prow["skip_reason"] = "top_n"
                        changed = True
                        break
                if changed:
                    self.write_csv(self.pending_file, pending_rows, fieldnames=self.pending_fields)
                print(f"[confirm skipped by top_n] {s.symbol} {s.side} score={s.score:.2f}")
