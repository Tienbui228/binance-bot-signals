...
def mark_pending_confirmed_fields(app, row: Dict, confirmed_ts_ms: Optional[int] = None, note: str = "signal confirmed") -> Dict:
    ...
    return app._normalize_row_for_fields(row, app.pending_fields)


def sync_confirmed_pending_row(app, setup_id: str, confirmed_ts_ms: Optional[int] = None, note: str = "signal confirmed"):
    rows = app.read_csv(app.pending_file)
    changed = False
    synced_row = None
    for idx, row in enumerate(rows):
        row_setup = row.get("setup_id") or row.get("pending_id", "")
        if row_setup == setup_id:
            updated = mark_pending_confirmed_fields(app, row, confirmed_ts_ms=confirmed_ts_ms, note=note)
            rows[idx] = updated
    ...
    if changed:
        if synced_row is not None:
            synced_row = app._enrich_pending_row_for_daily_review(synced_row)
            for idx, row in enumerate(rows):
                row_setup = row.get("setup_id") or row.get("pending_id", "")
                if row_setup == setup_id:
                    rows[idx] = synced_row
...
def save_signal(app, s: Signal):
    if not s.setup_id:
        s.setup_id = s.signal_id
    if not getattr(s, "market_regime", ""):
        s.market_regime = getattr(s, "btc_regime", "unknown")
    row = app._normalize_row_for_fields(asdict(s), app.signal_fields)
    app.append_csv(app.signals_file, row, fieldnames=app.signal_fields)
    synced_pending = sync_confirmed_pending_row(
        app,
        s.setup_id,
...
def save_pending(app, p: PendingSetup):
    ...
    pending_row.setdefault("dispatch_action", "not_evaluated")
...
def sync_pending_send_decision(app, setup_id: str, send_decision: str, skip_reason: str = "", sent_ts_ms: Optional[int] = None):
    ...
    if changed:
        ...
        if changed_row is not None:
            changed_row = app._enrich_pending_row_for_daily_review(changed_row)
            for idx, row in enumerate(rows):
                row_setup = row.get("setup_id") or row.get("pending_id", "")
                if row_setup == setup_id:
...
def close_pending(app, pending_id: str, status: str, close_reason: str, bars_waited: int = 0):
    rows = app.read_csv(app.pending_file)
    changed = False
    closed_row = None
    for idx, row in enumerate(rows):
        if row.get("pending_id") == pending_id and app._safe_pending_status(row) == "PENDING":
            row["status"] = status
    ...
    if changed:
        if closed_row is not None:
            closed_row = app._enrich_pending_row_for_daily_review(closed_row)
            for idx, row in enumerate(rows):
...
def close_signal(app, signal_row: Dict, outcome: str, r_multiple: float, bars_checked: int, close_reason: str, mfe_pct: float = 0.0, mae_pct: float = 0.0):
    results = app.read_csv(app.results_file)
    result_row = {
    ...
    for row in signals:
        if row.get("signal_id") == signal_row.get("signal_id"):
            row["status"] = outcome
    app.write_csv(app.signals_file, signals, fieldnames=app.signal_fields)

    print(f"[close] {signal_row.get('symbol','')} {signal_row.get('side','')} | {outcome} | r={r_multiple:.2f} | {close_reason}")
    if app.cfg.get("telegram", {}).get("send_close_notifications", True):
        try:
            app.telegram_send(app.format_close_message(signal_row, outcome, r_multiple, close_reason))
        except Exception as e:
