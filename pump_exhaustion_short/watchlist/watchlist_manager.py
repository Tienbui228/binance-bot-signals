...
CASE_FIELDS = [
    # Identity
    "case_id", "symbol", "case_state", "created_ts", "updated_ts",
    # Base
    ...
    "score_breakdown_quality", "score_market_pressure", "score_delivery",
    # Exclusion
...
class WatchlistManager:
    def __init__(self, data_dir: str):
        ...
        self._wl_path = os.path.join(data_dir, "..", "watchlist", "watchlist.json")
        # Support both relative (from project root) and absolute path
        if not os.path.isabs(self._wl_path):
            self._wl_path = os.path.join(os.path.dirname(__file__), "watchlist.json")
        self._lock = threading.Lock()
        self._cases: Dict[str, Dict] = {}  # keyed by case_id
        self._load()

        # CSV paths
        self._active_csv = os.path.join(data_dir, "cases_active.csv")
        self._closed_csv = os.path.join(data_dir, "cases_closed.csv")
        self._excluded_csv = os.path.join(data_dir, "cases_excluded.csv")
        self._discovery_log = os.path.join(data_dir, "discovery_log.csv")
        ...
        with self._lock:
            for existing in self._cases.values():
                if existing.get("symbol") == symbol and existing.get("case_state") != "EXCLUDED":
                    return False
            case_id = case.get("case_id", f"{symbol}_{int(time.time() * 1000)}")
